...
- react 에는 http 경로로 페이지 이동 못함. 즉 `화면 이동 불가`
- http 경로를 흔히 `router` 라고 칭함
- `router` 즉, 경로를 이동하려면 `react-router-dom` 을 사용해야함
...
- 인터넷 자원을 나타내는 고유 식별자.
- URI 에 "I" 가 Identifier인 것은 인터넷에 있는 자료의 ID를 뜻하는 것
- 즉, 다른 자료가 똑같은 이름을 가지고 있으면 안되며 그렇기 때문에 URI는 유일해야 함
...
```txt
1. HTTP (HyperText Transfer Protocal) : 웹 브라우저와 서버 컴퓨터간의 데이터 전송 규약

2. HTTPS (HTTP Secure) : http 에 보안 (SSL/TLS) 을 추가한 프로토콜

3. FTP (File Transfer Protocol) : 파일 전송에 사용하는 프로토콜.
웹 퍼블리싱을 하고 나면 FTP 로 서버에 파일을 업로드 하여 서비스함
대표적인 도구론 Filezilla, 알FTP (알FTP는 쓰지않기. 2012년에 개발 중단. 더 이상 기능개선이 되지 않음.)

4. SMTP (Simple Mail Transfer Protocol) : E-mail 전송

5. IMAP (Internet Message Access Protocol) : E-mail 수신 (서버에서 관리)

6. POP3 (Post Office Protocol 3) : E-mail 수신 (다운로드 후 로컬 관리)

7. DNS (Domain Name System) : 도메인 이름으로 IP 주소 변환

8. DHCP (Dynamic Host Cofiguration Protocol) : 동적 IP 할당
```

### 2.2 도메인

- localhost text 가 도메인임. (http:// 는 프로토콜)

```html
// localhost 가 도메인. (http://localhost:3000/todo/login?id=hong&pass=1234)
```

- 일반적으로 `홈페이지 주소` 로 이해
- DNS 서버가 있어야 함 (AWS 나 Vecel 주소 등)
...
http://localhost:3000/blog/design/1    블로그의 첫번째 디자인(REST API)
http://localhost:3000/blog/design/detail?id=1   Query String
```

## 4. Router 에 맞게 pages 폴더를 구성

- 라우터가 나와야 파일 폴더 생성해야함.
...
http://localhost:3000/about/mission    src/pages/mission/Index.jsx
http://localhost:3000/about/mission    src/pages/Mission.jsx

http://localhost:3000/about/team       src/pages/team/Index.jsx
http://localhost:3000/about/team       src/pages/Team.jsx
...
- 반드시 적용 순서는 `Router -> Routes -> Route` 순서 기억하자 (하나라도 어기면 힘들어짐)
...
## 7.3 Router 에 Param 전달하기 및 처리

- `Rest API 방식`
- BE 와 업무 진행시 param 이라는 단어를 알아야 함
- `경로/param`
- `http://localhost:3000/good/1` << 1 이 param
- `http://localhost:3000/good/2` << 2 이 param
- `http://localhost:3000/good/3` << 3 이 param
- `http://localhost:3000/blog/design/1` << 1 이 param
...
```jsx
<Route path="/" element={<IndexPage first={"첫 페이지입니다"} />}></Route>
```
